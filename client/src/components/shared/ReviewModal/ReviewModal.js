// src/components/admin/ReviewModal/ReviewModal.js
...
const ReviewModal = ({ 
  ...
  onSubmit, 
  initialData = null,
  contentType = 'review' // Add contentType prop with default 'review'
}) => {
  ...
  // Select the appropriate tabs based on content type
  ...
  // Form submission - Updated for S3
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add checkbox fields for news articles
      if (contentType === 'news' && formData.featured !== undefined) {
        formDataToSubmit.append('featured', formData.featured.toString());
      }
      
      // Add content based on content type
      if (contentType === 'review') {
        // For reviews, stringify the content object
        ...
        // For news, content is a plain string
      ...
      // Add featured image if it exists - Files will be uploaded to S3 by backend
      ...
      // Add gallery images if they exist - Files will be uploaded to S3 by backend
      ...
      console.log(`Submitting ${contentType} with payload:`, {
        endpoint: initialData ? `/api/${contentType === 'news' ? 'news' : 'reviews'}/${initialData._id}` : `/api/${contentType === 'news' ? 'news' : 'reviews'}`,
        method: initialData ? 'PUT' : 'POST',
        fields: [...formDataToSubmit.keys()]
      });
      
      // Submit the form using the provided onSubmit function
      await onSubmit(formDataToSubmit);
      
      dispatch(addNotification({
        type: 'success',
        message: initialData ? `${contentType === 'news' ? 'Article' : 'Review'} updated successfully` : `${contentType === 'news' ? 'Article' : 'Review'} created successfully`
      }));
      ...
      dispatch(addNotification({
        type: 'error',
        message: error.message || `Failed to save ${contentType}`
      }));
      setErrors(prev => ({
        ...prev,
        submit: error.message || `Failed to save ${contentType}. Please try again.`
      }));
    } finally {
