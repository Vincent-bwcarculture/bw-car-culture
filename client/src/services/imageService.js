// src/services/imageService.js - Enhanced with User Listing Support
...
class ImageService {
  constructor() {
    ...
    this.userEndpoint = '/api/user'; // NEW: For user-specific uploads
...
/**
 * NEW: Upload multiple images for USER LISTINGS specifically
 * @param {File[]} files - Array of image files
 * @param {string} folder - The S3 folder to upload to (default: 'user-listings')  
 * @param {Function} onProgress - Progress callback
 * @returns {Promise<Object[]>} - Array of upload results
 */
async uploadMultipleForUser(files, folder = 'user-listings', onProgress) {
  if (!files || files.length === 0) {
  ...
  validFiles.forEach((file, index) => {
    const fileObj = file instanceof File ? file : file.file;
    formData.append('images', fileObj); // User endpoint expects 'images' field
    console.log(`🖼️ USER: Added file ${index + 1}: ${fileObj.name} (${fileObj.type}, ${(fileObj.size / 1024).toFixed(1)}KB)`);
  });

  try {
    const endpoint = `${this.userEndpoint}/upload-images`;
    console.log(`🖼️ USER: Sending upload request to: ${endpoint}`);

    const response = await axios.post(endpoint, formData, {
      headers: {
        ...this.getHeaders(true) // FormData headers
      },
      ...
      onUploadProgress: (progressEvent) => {
        ...
        console.log(`🖼️ USER: Upload progress: ${progress}%`);
        onProgress?.(progress);
      }
    });
  ...
  /**
   * NEW: Validate image file for user uploads
  ...
  /**
   * NEW: Create image preview URL
   * @param {File} file - File object
   * @returns {string} Object URL for preview
   */
  createPreviewUrl(file) {
    if (!(file instanceof File)) {
      console.error('🖼️ Cannot create preview - invalid file object');
      return null;
    }
    ...
    try {
      ...
      console.error('🖼️ Error creating preview URL:', error);
      return null;
    }
  }

  /**
   * NEW: Clean up preview URL to prevent memory leaks
  ...
  revokePreviewUrl(url) {
    if (url && url.startsWith('blob:')) {
      try {
        ...
        console.warn('🖼️ Error revoking preview URL:', error);
      }
    }
  }
}
