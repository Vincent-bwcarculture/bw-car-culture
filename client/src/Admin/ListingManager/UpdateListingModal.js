  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setFormData({
        ...
        listingQuality: clonedData.listingQuality ?? 50,
        ...
        // Transit status
        transit: {
          isInTransit: clonedData.transit?.isInTransit || false,
          destinationCountry: clonedData.transit?.destinationCountry || 'BW',
          eta: clonedData.transit?.eta ? new Date(clonedData.transit.eta).toISOString().split('T')[0] : ''
        },

        // Admin-only private records
        adminNotes: {
          supplierName: clonedData.adminNotes?.supplierName || '',
          supplierPrice: clonedData.adminNotes?.supplierPrice || '',
          margin: clonedData.adminNotes?.margin || '',
          purchaseDate: clonedData.adminNotes?.purchaseDate ? new Date(clonedData.adminNotes.purchaseDate).toISOString().split('T')[0] : '',
          notes: clonedData.adminNotes?.notes || ''
        }
      });
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    console.log('formData.specifications:', formData.specifications);
    ...
    requiredFields.forEach(field => {
      ...
      if (isEmpty) {
        ...
        console.error(`❌ ${field.label} validation failed:`, fieldValue);
      } else {
        console.log(`✅ ${field.label} validated:`, fieldValue);
      }
    });
    ...
    if (formData.specifications?.year) {
      ...
      if (isNaN(year) || year < 1900 || year > new Date().getFullYear() + 1) {
        newErrors['specifications.year'] = 'Please enter a valid year';
      }
    }
    ...
    // Validate images
    const totalImages = (images?.length || 0) + (newImages?.length || 0) - (imagesToDelete?.length || 0);
    if (totalImages <= 0) {
      newErrors.images = 'At least one image is required';
    }
    ...
    // Prepare submission data - ENSURE correct field mapping
    const submitData = {
      // Basic fields
      ...
      model: formData.specifications?.model?.trim(),  // ← Map nested to flat  
      year: parseInt(formData.specifications?.year),  // ← Map nested to flat
      
      // Additional specs
      mileage: formData.specifications?.mileage ? parseInt(formData.specifications.mileage) : undefined,
      ...
      // Keep nested specifications for compatibility
    ...
    console.log('Make:', submitData.make);
    console.log('Model:', submitData.model);
    console.log('Year:', submitData.year);
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                >
                  ...
                  <option value="Quad Bike">Quad Bike / ATV</option>
...
<input
  type="text"
  id="make"
  name="specifications.make"  // ← FIXED: Now consistent
  value={getSafeFormValue(formData, 'specifications.make', '')}
  onChange={handleNestedChange}  // ← FIXED: Now consistent with Model/Year
  placeholder="Make"
...
/>
          ...
          <section className={`form-section ${activeTab === 'records' ? 'active' : ''}`}>
            ...
            <div className="form-grid">
              ...
              <div className="form-group full-width">
                <label htmlFor="adminPrivateNotes">Private Notes</label>
                <textarea
                  id="adminPrivateNotes"
